...
def yield_tokens(data_iter, tokenizer, index):
    #check which line is calling the decode which crashes at encode error
...
##    model.train()
##    
##    loss = run_epoch((Batch(b[0], b[1], pad_idx) for b in train1),
##                     model,
...
##    out = greedy_decode(model, src, src_mask, max_len=2,
...
##GPT CONTEXT-RESPONSE RUNS

##train_gpt = data_loading.get_train_dataloader()
##valid_gpt = data_loading.get_valid_dataloader()
##all_vocab = data_loading.all_vocab
##
##pad_idx = all_vocab["<blank>"]
##                                                         
###print(src_pipeline.get_stoi())
##
##
##criterion = LabelSmoothing(size=len(all_vocab),
##                           padding_idx=pad_idx, smoothing=0.1)
##gpt_model = make_gpt_model(len(all_vocab),
##                           len(all_vocab), N=6)
##gpt_model_opt = NoamOpt(gpt_model.src_embed[0].d_model, 1, 400,
##                    torch.optim.Adam(gpt_model.parameters(), lr=0,
##                                     betas=(0.9, 0.98), eps=1e-9))
##gpt_model_path = "test_gpt_model.pt"
##model_load = False
##epoch = 0
##start_time = time.time()

#TODO just run training
...
##            print("path exists")
##            chkpt = torch.load(gpt_model_path)
##            gpt_model.load_state_dict(chkpt['model_state_dict'])
##            gpt_model_opt.optimizer.load_state_dict(chkpt['optimizer_state_dict'])
##            epoch = chkpt['epoch']
##            loss = chkpt['loss']
##            model_load = True
##            
##    gpt_model.train()
##
##    loss = run_gpt_epoch((Batch(b[0], b[1], pad_idx) for b in train_gpt),
##                     gpt_model,
##                     SimpleLossCompute(gpt_model.generator, criterion,
##                                       gpt_model_opt), epoch)
##    gpt_model.eval()
##    print(run_gpt_epoch((Batch(b[0], b[1], pad_idx) for b in valid_gpt),
##                        gpt_model,
##                        SimpleLossCompute(gpt_model.generator, criterion,
##                                          gpt_model_opt), epoch))
##    
...
##            'optimizer_state_dict': gpt_model_opt.optimizer.state_dict(),
##            'loss': loss
##            }, gpt_model_path)
##    epoch += 1
...
##valid = data_loading.get_valid_dataloader()
##
##v_batch = (Batch(b[0], b[1], pad_idx) for b in valid)
##
##print(v_batch)
...
##
##sent = "How does the human immune system function?"
##
##src = torch.LongTensor([all_vocab.get_stoi()[x] for x in sent.split()])
##src = Variable(src)
##
##
##sos_id = torch.tensor([0])
##eos_id = torch.tensor([1])
##mem_src = torch.cat([sos_id, torch.tensor(all_vocab(tokenizer(sent)),
##                                          dtype=torch.int64),
##                     eos_id], 0,)
###print(mem_src)
##max_padding = 512
...
##
##
##out = gpt_greedy_decode(gpt_model, memory, src_mask, max_len=100,
...
#GPT ENCODER DECODER STRUCTURE TRAINING RUNS
...
##                                     betas=(0.9, 0.98), eps=1e-9))
##gpt_modeld_path = "test_gpt_model_d.pt"
##model_load = False
##epoch = 0
##start_time = time.time()
##
...
##    log.p("\n\n-------------------------------------------------------START------------------------------------------------------------------------------------------------------------------------------------------------\n\n")
##    if model_load == False:
##        if os.path.exists(gpt_modeld_path):
##            print("path exists")
##            chkpt = torch.load(gpt_modeld_path)
...
##    print("Elapsed time: ", round(elapsed, 2))
####    
####GPT ENCODER DECODER STRUCTURE PREDICTION RUNS
##
##if os.path.exists(gpt_modeld_path):
##    print("path exists")
##    chkpt = torch.load(gpt_modeld_path)
...
#training_run(tweet_img, "tweet_image.pt")

prediction_run(tweet_img, "tweet_image.pt",
               "rallybook: is a beacon of hope")
