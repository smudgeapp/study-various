...
with open(train_set, 'r', encoding='utf-8') as f:
    data = json.load(f)
    ctx = [ct['type'] for ct in data]
    resp = [res['tweet'] for res in data]
...
train_src_len = len_vocab(train_set, 'type')
train_tgt_len = len_vocab(train_set, 'tweet')

valid_src_len = len_vocab(valid_set, 'type')
valid_tgt_len = len_vocab(valid_set, 'tweet')
...
def get_max_padding(file_path):
    ...
    with open(file_path, encoding='utf-8') as f:
        data = json.load(f)
        ctx = [ct['type'].split(' ') for ct in data]
        ctx_list = []
        for ct in ctx:
            ctx_list.append(len(ct))
        src_len = max(ctx_list)
        
        resp = [res['tweet'].split(' ') for res in data]
        res_list = []
        for res in resp:
