...
ArrayXXf DenseLayer::backwardW2V(ArrayXXf backInput) {
	//std::cout << "fwd output " << this->forwardInput << std::endl;
	//std::cout << "bwdinput " << backInput << std::endl;
	MatrixXf bwdWt = this->forwardInput.transpose().matrix() * backInput.matrix();
	this->dWeightsW2V = bwdWt.array();
	//std::cout << "check wts: " << this->dWeightsW2V << std::endl;
	MatrixXf bwdInput = backInput.matrix() * this->wtMat.transpose().matrix();
	this->dInputW2V = bwdInput.array();
	//std::cout << "dinputw2v " << this->dInputW2V << std::endl;
	this->dBiasW2V = backInput;
	return bwdInput.array();
}
