...
*Note: It should not be a problem if a single paragraph vector has multiple representative vectors, since different sets of factors could have a similar consequence. As long as there is sufficient variation in the similarity output, this should not be a bother. However, if the variation is minimal, for instance, the output keeps returning the same one or two paragraphs, then it cannot be said that a set of factors is effecting the stock indicator consequence, since there is next to no change from one vector to the other.*
