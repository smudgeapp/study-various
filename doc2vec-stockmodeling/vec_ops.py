...
## different.
    ...
    def run_test(self, d2vmodel, sents_vecs, corpus, vecs=0, polar_sents=1, topn=10,
                 epochs=1000, write_to=None):
        if vecs > 0:
            if polar_sents > 0:
                self.run_polar_vec_test(d2vmodel, sents_vecs, corpus, topn=topn,
                                        epochs=epochs, write_to=write_to)
            else:
                self.run_plain_vec_test(d2vmodel, sents_vecs, corpus, topn=topn,
                                     epochs=epochs, write_to=write_to)
        else:                
            if polar_sents > 0:
                self.run_polar_sent_test(d2vmodel, sents_vecs, corpus, topn=topn,
                                         epochs=epochs, write_to=write_to)
            else:
                self.run_plain_sent_test(d2vmodel, sents_vecs, corpus, topn=topn,
                                         epochs=epochs, write_to=write_to)
        


    def run_polar_sent_test(self, d2vmodel, sents, corpus, topn=10, epochs=1000,
                            write_to=None):
        if write_to:
            with open(write_to, 'w') as f:
                pass
        ...
        for i, sent in enumerate(sents):
            pos_vec = d2vmodel.infer_vector(sent['pos_vec'], epochs=epochs)
            neg_vec = d2vmodel.infer_vector(sent['neg_vec'], epochs=epochs)
            sim_pos = d2vmodel.dv.most_similar(pos_vec, topn=topn)
            sim_neg = d2vmodel.dv.most_similar(pos_vec, topn=topn)
            if write_to:
                with open(write_to, 'a') as f:
    ...
    def run_polar_vec_test(self, d2vmodel, vecs, corpus, topn=10, epochs=1000,
                           write_to=None):
        if write_to:
            with open(write_to, 'w') as f:
                pass
    ...
    def run_plain_sent_test(self, d2vmodel, sents, corpus, topn=10, epochs=1000,
                            write_to=None):
        if write_to:
            with open(write_to, 'w') as f:
                pass
        ...
        for i, sent, in enumerate(sents):
            vec = d2vmodel.infer_vector(sent, epochs=epochs)
    ...
    def run_plain_vec_test(self, d2vmodel, vecs, corpus, topn=10, epochs=1000,
                            write_to=None):
        if write_to:
            with open(write_to, 'w') as f:
                pass
